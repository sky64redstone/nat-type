...
int stun_test(const char* host, int port, int change_ip, int change_port, mapped_addr_t* result) {
    int sock;
...
int main() {
    ...
    // Test I
    if (!stun_test("stun1.l.google.com", 19302, 0, 0, &m1)) {
        printf("UDP Blocked\n");
        return 0;
    }
    ...
    if (stun_test("stun1.l.google.com", 19302, 1, 1, &m2)) {
        printf("Open Internet or Full Cone NAT\n");
        return 0;
    }

    // Test I to different server
    if (!stun_test("stun2.l.google.com", 19302, 0, 0, &m2)) {
    ...
    if (stun_test("stun1.l.google.com", 19302, 0, 1, &m2)) {
